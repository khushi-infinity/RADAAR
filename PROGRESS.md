...
| 2026-09-19 | Paytm judge guide (docs/PAYTM-GUIDE.md): Paytm surfaces table (live vs swap-in), the 4 judge questions (useful/impactful/scalable/best-use) answered with numbers, slide-by-slide deck alignment, 5-beat business demo, integration priority order | Paytm evaluators score merchant value + scalability + honesty about the data path; the "schema swap not architecture change" framing converts the synthetic-data weakness into a testability strength |
| 2026-09-19 | Business-judges PPT (docs/RADAAR-Business-Pitch.pptx, regenerable via npm run deck): 10-slide investor narrative — merchant story → invisible leak → the gap → 4-question product → measured outcome → why-now/Paytm → market & model → the ask. Speaker notes on every slide | Business judges buy narrative + numbers, not architecture; deck front-loads the ₹ leak and the +6.1% outcome, tech proof compressed to one slide |
